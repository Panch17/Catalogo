...
OUTPUT_HTML = os.path.join(SCRIPT_DIR, "index1.html")
...
{% endif %}
  ...
  <!-- Modal Minijuego -->
<div class="modal fade" id="minijuegoModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content text-center">
      <div class="modal-header bg-warning">
        <h5 class="modal-title">🎮 Encuentra el Tesoro</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
      </div>
      <div class="modal-body">
        <p class="mb-3">Haz clic en una casilla para encontrar el 🎁. ¡Tienes 5 intentos!</p>
        <div id="juegoGrid" class="d-grid gap-2" style="grid-template-columns: repeat(5, 1fr);"></div>
        <p id="mensajeJuego" class="mt-3 fw-bold"></p>
      </div>
    </div>
  </div>
</div>
...
function iniciarMinijuego() {
  const grid = document.getElementById('juegoGrid');
  const mensaje = document.getElementById('mensajeJuego');
  grid.innerHTML = '';
  mensaje.innerText = '';
  
  const totalCeldas = 25;
  const intentosMax = 5;
  let intentos = 0;
  const tesoro = Math.floor(Math.random() * totalCeldas);
  ...
  for (let i = 0; i < totalCeldas; i++) {
    ...
    celda.addEventListener('click', function handler() {
      intentos++;
      if (i === tesoro) {
        celda.innerText = '🎁';
        celda.style.backgroundColor = '#90ee90';
        mensaje.innerText = `🎉 ¡Encontraste el tesoro en ${intentos} intento(s)!`;
        grid.querySelectorAll('.juego-celda').forEach(c => c.removeEventListener('click', handler));
      } else {
        celda.innerText = '❌';
        celda.style.backgroundColor = '#f8d7da';
        celda.removeEventListener('click', handler);
        if (intentos >= intentosMax) {
          mensaje.innerText = `😢 Se acabaron los intentos. El tesoro estaba en la casilla #${tesoro + 1}.`;
          grid.querySelectorAll('.juego-celda').forEach(c => c.removeEventListener('click', handler));
        }
      }
    });
  ...
  new bootstrap.Modal(document.getElementById('minijuegoModal')).show();
}
// Presiona la tecla G para abrir el minijuego
